...
const hotelFunctions = {

  fetchHotel: async () => {
    return await fetch('https://5df9cc6ce9f79e0014b6b3dc.mockapi.io/hotels/tokyo', { mode: 'no-cors' })
      .then(res => res.json())
      .then(data => {
        return data
      }).catch(err => {
  ...
  fetchPrice: async (currency) => {
    return await fetch(`https://5df9cc6ce9f79e0014b6b3dc.mockapi.io/hotels/tokyo/1/${currency}`, { mode: 'no-cors' })
      .then(res => res.json())
      .then(data => {
        return data
      }).catch(err => {
  ...
  useHotelAPI: (currency) => {
    const [hoteldata, setHoteldata] = useState([])

    const fetchHoteldata = useCallback(async () => {
      const [infoData, originalPriceData] = await Promise.all([
        hotelFunctions.fetchHotel(),
        hotelFunctions.fetchPrice(currency)
      ])
